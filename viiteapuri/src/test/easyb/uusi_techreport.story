...
def perusviite = [nimi : "techViite", tyyppi : "techreport", author : "juri", title : "testaamisen iloa", year : "1995", institution : "HY"]
...
description "Käyttäjä voi luoda uuden techreport-viitteen"

scenario "käyttäjä kykenee lisäämään uuden techreport-viitteen", {
...
scenario "käyttäjä tietää mitkä kentät ovat pakollisia", {
    ...
    then 'käyttäjä saa tiedon pakollisista kentistä', {
        ...
        def pakolliset = ViiteTyyppi.techreport.getPakolliset()
...
scenario "käyttäjän tulee täyttää pakolliset kentät", {
    ...
    when 'pakollinen kenttä jätetään tyhjäksi', {
        when(io.lueRivi(contains("nimi"))).thenReturn("", "techViite")
        perustilanne()        
        app.run()
    }
