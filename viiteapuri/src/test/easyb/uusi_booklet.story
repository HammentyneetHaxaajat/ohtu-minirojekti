...
def perusviite = [nimi : "bkltViite", tyyppi : "booklet", title : "testaamisen iloa"]
...
description "Käyttäjä voi luoda uuden booklet-viitteen"

scenario "käyttäjä kykenee lisäämään uuden booklet-viitteen", {
...
scenario "käyttäjä tietää mitkä kentät ovat pakollisia", {
    ...
    then 'käyttäjä saa tiedon pakollisista kentistä', {
        ...
        def pakolliset = ViiteTyyppi.booklet.getPakolliset()
...
scenario "käyttäjän tulee täyttää pakolliset kentät", {
    ...
    when 'pakollinen kenttä jätetään tyhjäksi', {
        when(io.lueRivi(contains("nimi"))).thenReturn("", "bkltViite")
        perustilanne()        
        app.run()
    }
