...
def perusviite = [nimi : "phdViite", tyyppi : "phdthesis", author : "juri", title : "testaamisen iloa", year : "1995", school : "HY"]
...
description "Käyttäjä voi luoda uuden phdthesis-viitteen"

scenario "käyttäjä kykenee lisäämään uuden phdthesis-viitteen", {
...
scenario "käyttäjä tietää mitkä kentät ovat pakollisia", {
    ...
    then 'käyttäjä saa tiedon pakollisista kentistä', {
        ...
        def pakolliset = ViiteTyyppi.phdthesis.getPakolliset()
...
scenario "käyttäjän tulee täyttää pakolliset kentät", {
    ...
    when 'pakollinen kenttä jätetään tyhjäksi', {
        when(io.lueRivi(contains("nimi"))).thenReturn("", "phdViite")
        perustilanne()        
        app.run()
    }
