...
def perusviite = [nimi : "mtViite", tyyppi : "mastersthesis", author : "juri", title : "testaamisen iloa", year : "1995", school : "HY"]
...
description "Käyttäjä voi luoda uuden mastersthesis-viitteen"

scenario "käyttäjä kykenee lisäämään uuden mastersthesis-viitteen", {
...
scenario "käyttäjä tietää mitkä kentät ovat pakollisia", {
    ...
    then 'käyttäjä saa tiedon pakollisista kentistä', {
        ...
        def pakolliset = ViiteTyyppi.mastersthesis.getPakolliset()
...
scenario "käyttäjän tulee täyttää pakolliset kentät", {
    ...
    when 'pakollinen kenttä jätetään tyhjäksi', {
        when(io.lueRivi(contains("nimi"))).thenReturn("", "mtViite")
        perustilanne()        
        app.run()
    }
