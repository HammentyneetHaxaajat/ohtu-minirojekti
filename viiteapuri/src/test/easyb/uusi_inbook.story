...
def perusviite = [nimi : "inbViite", tyyppi : "inbook", author : "jurppi", publisher : "juristamo", pages : "2", title : "testaamisen iloa", year : "1995"]
...
description "Käyttäjä voi luoda uuden inbook-viitteen"

scenario "käyttäjä kykenee lisäämään uuden inbook-viitteen", {
...
scenario "käyttäjä tietää mitkä kentät ovat pakollisia", {
    ...
    then 'käyttäjä saa tiedon pakollisista kentistä', {
        ...
        def pakolliset = ViiteTyyppi.inbook.getPakolliset()
...
scenario "käyttäjän tulee täyttää pakolliset kentät", {
    ...
    when 'pakollinen kenttä jätetään tyhjäksi', {
        when(io.lueRivi(contains("nimi"))).thenReturn("", "inbViite")
        perustilanne()        
        app.run()
    }
